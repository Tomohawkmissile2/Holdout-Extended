	...
	for _,building in ipairs( buildings ) do 
		if building:IsTower() then
			...
			if fortificationAbility then
				fortificationAbility:SetLevel( (self.nRoundNumber /2)+1)
			end
			
			if not building:HasAbility( "tower_healing_aura" ) then building:AddAbility( "tower_healing_aura" ) end
			local fortificationAbility = building:FindAbilityByName( "tower_healing_aura" )
			if fortificationAbility then
				fortificationAbility:SetLevel( (self.nRoundNumber /2)+1)
			end
